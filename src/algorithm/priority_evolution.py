...
def q(cfg, act_fn=nn.ELU()):
    ...
    return nn.Sequential(nn.Linear(cfg.latent_dim + cfg.action_dim, cfg.mlp_dim),
                         ...
                         nn.LayerNorm(cfg.mlp_dim), nn.Tanh(),
                         # squash to [-1, 1]-centered around 0, makes convergence and training faster
...
class ReplayBuffer():
    ...
    def sample(self):
        ...
        next_obs = torch.empty((self.cfg.horizon + 1, self.cfg.batch_size, *next_obs_shape), dtype=obs.dtype,
                               device=obs.device)
        action = torch.empty((self.cfg.horizon + 1, self.cfg.batch_size, *self._action.shape[1:]), dtype=torch.float32,
                             device=self.device)
...
class Config:
    def __init__(self):
        ...
        self.obs_shape = (4,)  # 4-dimensional state
        self.action_dim = 2
...
class Episode:
    def __init__(self):
        self.obs = torch.randn(cfg.episode_length + 1, *cfg.obs_shape)  # +1 for last_obs
        self.action = torch.randn(cfg.episode_length, cfg.action_dim)
        self.reward = torch.randn(cfg.episode_length)

# ReplayBuffer from your code
# Assuming it's defined as ReplayBuffer(cfg)
...
print("Initial priorities:\n", buffer._priorities[:buffer.idx])

# Sample a batch
obs, next_obs, action, reward, idxs, weights = buffer.sample()
print("\nSampled indices:", idxs)
print("Sampled weights:", weights)

# Simulate TD-error updates
new_priorities = torch.rand(cfg.batch_size, 1) * 2
buffer.update_priorities(idxs, new_priorities)

# Check updated priorities
print("\nUpdated priorities at sampled indices:")
print(buffer._priorities[idxs])
...
class DummyConfig:
    def __init__(self):
        ...
        self.frame_stack = 1
        self.modality = 'state'
        self.capacity = min(self.train_steps, self.max_buffer_size)
...
class DummyReplayBuffer:
    ...
    def add_dummy_episode(self):
        if self._full:
            max_priority = self._priorities.max().item()
        else:
            max_priority = 1.0 if self.idx == 0 else self._priorities[:self.idx].max().item()

        mask = torch.arange(self.cfg.episode_length) >= self.cfg.episode_length - self.cfg.horizon
        new_priorities = torch.full((self.cfg.episode_length,), max_priority)
        new_priorities[mask] = 0
        end_idx = self.idx + self.cfg.episode_length
        if end_idx <= self.capacity:
            self._priorities[self.idx:end_idx] = new_priorities
        else:
            over = end_idx - self.capacity
            self._priorities[self.idx:] = new_priorities[:-over]
            self._priorities[:over] = new_priorities[-over:]

        self.idx = (self.idx + self.cfg.episode_length) % self.capacity
        self._full = self._full or self.idx == 0

    def sample(self):
        probs = self._priorities.clone() ** self.cfg.per_alpha
        probs /= probs.sum()
        return np.random.choice(self.capacity, self.cfg.batch_size, p=probs.numpy())

    def update_priorities(self, idxs, priorities):
        self._priorities[idxs] = priorities + self._eps

buffer = DummyReplayBuffer(cfg)
sampling_record = np.zeros((cfg.capacity, 100))
priority_track = []

for step in range(100):
    buffer.add_dummy_episode()
    idxs = buffer.sample()
    sampling_record[idxs, step] += 1
    buffer.update_priorities(torch.tensor(idxs), torch.rand(len(idxs)))  # simulate updates
    priority_track.append(buffer._priorities.clone().numpy())

# Convert to numpy
priority_track = np.array(priority_track)  # (steps, capacity)

# Plotting
fig, axs = plt.subplots(1, 2, figsize=(14, 6))

# Heatmap
sns.heatmap(sampling_record, cmap='viridis', ax=axs[0])
axs[0].set_title("Sampling Frequency Heatmap")
axs[0].set_xlabel("Training Steps")
axs[0].set_ylabel("Buffer Index")

# Final priority bar chart
axs[1].bar(np.arange(cfg.capacity), buffer._priorities.numpy(), color='tomato')
axs[1].set_title("Final Priorities")
axs[1].set_xlabel("Buffer Index")
axs[1].set_ylabel("Priority")
...
plt.plot(priority_track[:, :min(50, cfg.capacity)])  # visualize first 50 entries
plt.title("Priority Evolution Over Time (first 50 entries)")
...
# Parameters
capacity = 100  # Size of the buffer
num_updates = 50  # Number of episodes added
horizon = 5      # How many timesteps in each episode are zero-prioritized
...
for update_idx in range(num_updates):
    # Simulate a new episode being added
    start = (update_idx * horizon) % capacity
    end = start + horizon

    # Assign max priority to new transitions, rest zero (simulate TD-MPC logic)
    new_priorities = np.zeros(horizon)
    if update_idx > 0:
        max_priority = priorities.max()
    else:
        max_priority = 1.0
    new_priorities[:-1] = max_priority

    # Update priorities in the buffer
    priorities[start:end] = new_priorities
    priority_history.append(priorities.copy())

    # Plot current frame
    plt.figure(figsize=(10, 2))
    plt.title(f"Priorities at Update {update_idx+1}")
    plt.bar(np.arange(capacity), priorities, color='skyblue')
    plt.xlabel("Buffer Index")
    plt.ylabel("Priority")
    plt.ylim(0, 1.1)
    plt.tight_layout()
    filename = f"frames/frame_{update_idx:03d}.png"
...
imageio.mimsave("priority_evolution.gif", images, duration=0.2)

print("✅ GIF saved as 'priority_evolution.gif'")
